...
- **Story:** WLA-13-03 - Correlate live agent sessions to stories
- **Status:** done
- **Date:** 2026-07-04
...
1. **The live proof (06:01:59Z):** a real codex session, launched
   inside a tmux session created for the occasion, in the rails
   fixture repo, on a story flipped in-progress for it. The
   HoldSpeak hook reported it; `dw sessions` resolved it:
   `on_story WSH-1-02`, tmux addressing
   `dw-correlation-proof:0.%0`, `stale: false` — asserted, not
   eyeballed. The same capture shows the whole desk honestly: old
   claude sessions `off_rails` in the HoldSpeak repo, the
   yesterday's proof sessions `idle_on_rails` and stale.
2. **Suite (06:02:12Z):** 149 core tests including the two new
   correlator tests — all five outcomes (`on_story`, `ambiguous`,
   `idle_on_rails`, `off_rails`, `unreadable`) plus staleness
   against an injected clock, the frozen session key set, and the
   three registry failure shapes (absent, wrong version, garbage).

Amendments recorded in docs/mission-control.md §2 in this commit:
the registry's verified live format (`{"version": 1, "sessions":
{...}}` — it pins itself, and the correlator refuses politely on a
foreign version); the fifth outcome `unreadable` (rails markers
present, roadmap unparseable — unknown beats guessed in both
directions); and correlation as its own document
(`sessions_schema` 1) rather than a key grafted onto the frozen
feed. Test-fixture confession, kept: my first `unreadable` fixture
broke the rails markers instead of the parse and correctly came
back `off_rails` — the test caught the test.

### Captured run — 2026-07-04T06:01:59Z

- **Command:** `bash -c 
set -e
echo "== a real codex session, in tmux, on an in-progress story =="
.githooks/dw sessions | grep dw-correlation || true
.githooks/dw sessions --json | python3 -c "
import json, sys
doc = json.load(sys.stdin)
assert doc[\"registry\"] == \"ok\" and doc[\"sessions_schema\"] == 1
live = [s for s in doc[\"sessions\"] if (s[\"tmux\"] or {}).get(\"session\") == \"dw-correlation-proof\"]
assert live, \"the tmux session must be in the registry\"
s = live[0]
print(\"agent:\", s[\"agent\"])
print(\"correlation:\", s[\"correlation\"])
print(\"story:\", s[\"stories\"][0][\"story_id\"], \"-\", s[\"stories\"][0][\"title\"])
print(\"tmux:\", s[\"tmux\"])
print(\"stale:\", s[\"stale\"])
assert s[\"correlation\"] == \"on_story\"
assert s[\"stories\"][0][\"story_id\"] == \"WSH-1-02\"
assert s[\"stale\"] is False
print(\"PASS: live session resolved to its in-progress story with tmux addressing\")"
echo
echo "== the full live desk picture (all outcomes, honestly) =="
.githooks/dw sessions
`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** daf656fbf6a3649470d95668958a7552cfbe79b4

```text
== a real codex session, in tmux, on an in-progress story ==
codex	WSH-1-02	/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/fdc6877b-9071-406e-94f9-c2020d111439/scratchpad/codex-rider-fixture	tmux dw-correlation-proof:0.%0
agent: codex
correlation: on_story
story: WSH-1-02 - Add payment provider
tmux: {'pane': '%0', 'session': 'dw-correlation-proof', 'window': '0'}
stale: False
PASS: live session resolved to its in-progress story with tmux addressing

== the full live desk picture (all outcomes, honestly) ==
claude	off_rails	/Users/karol/dev/tools/HoldSpeak	awaiting-response stale tmux gate:0.%0
claude	off_rails	/Users/karol/dev/tools/HoldSpeak	awaiting-response stale
codex	WSH-1-02	/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/fdc6877b-9071-406e-94f9-c2020d111439/scratchpad/codex-rider-fixture	stale
codex	WSH-1-02	/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/fdc6877b-9071-406e-94f9-c2020d111439/scratchpad/codex-rider-fixture	stale
codex	WSH-1-02	/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/fdc6877b-9071-406e-94f9-c2020d111439/scratchpad/codex-rider-fixture	tmux dw-correlation-proof:0.%0
```

### Captured run — 2026-07-04T06:02:12Z
