...
- **Story:** WLA-13-04 - Emit the events worth showing
- **Status:** done
- **Date:** 2026-07-04
...
1. **Suite (06:09:29Z):** 153 core tests including the four new
   event tests — every taxonomy event fires at its rail moment
   (phase created, story created as a from-null flip, the
   in-progress flip carrying its real previous status, the capture
   with exit code, the contract with its stories); the gate run
   emits pass-or-refusal with the rule id; the content audit
   proves a rogue caller's diff, transcript, and ten-thousand-
   character prompt never reach the log; append-only holds and
   emission never raises (a .git-less root is a silent no-op,
   oversized values truncate).
2. **The self-recording log (06:09:40Z):** the log went live on
   this repo mid-story, so this capture shows it recording the
   story that built it — the suite run as `evidence_capture
   exit_code=0`, then `story_status WLA-13-04 from=in-progress
   to=done`, asserted.

One arc line the frozen evidence cannot contain, recorded here:
after this file is committed, the log gains `contract_generated`
and `gate_pass` for the very commit that shipped it — verifiable
any time with `dw events` on this repo.

Emission points, for the record: one `apply_plan` (flips, creates,
phase open/close), one `run_capture`, one `write_contract`, one
`run_gate` — every path (CLI, MCP, workbench) passes through them.
The phase's deferred persistence decision resolves to the
contract's JSONL file; no volume data argued for more.

### Captured run — 2026-07-04T06:09:29Z

- **Command:** `python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 2e121e8c08d1760a3b16190d40e6e1267530bbf4
...
----------------------------------------------------------------------
Ran 153 tests in 10.469s

OK
```

### Captured run — 2026-07-04T06:09:40Z

- **Command:** `bash -c 
echo "== the log, live on this repo, showing this very story happening =="
.githooks/dw events
echo
.githooks/dw events --json | python3 -c "
import json, sys
events = json.load(sys.stdin)
kinds = [e[\"event\"] for e in events]
assert \"evidence_capture\" in kinds, kinds
assert any(e[\"event\"] == \"story_status\" and e[\"story\"] == \"WLA-13-04\" and e[\"detail\"][\"to\"] == \"done\" for e in events)
print(\"PASS: the event log recorded this story flipping itself done\")"
`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 2e121e8c08d1760a3b16190d40e6e1267530bbf4
...
```text
== the log, live on this repo, showing this very story happening ==
2026-07-04T06:09:39Z	evidence_capture	WLA-13-04	exit_code=0 timestamp=2026-07-04T06:09:29Z
2026-07-04T06:09:39Z	story_status	WLA-13-04	from=in-progress to=done

PASS: the event log recorded this story flipping itself done
```
