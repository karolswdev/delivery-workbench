...
- [ ] Desktop and mobile overview render verdict, selected project,
...
- [ ] Attention and ambiguity are at least as prominent as ready state;
  the panel never presents a commit button or executes a recommendation.
- [ ] The canonical agent brief instructs every rider to call status once
  before work, explains its exit contract, and points to specialist
  surfaces for depth.
- [ ] Regeneration is idempotent and all committed agent copies match the
  one canon; existing users' unmanaged prose remains untouched.
- [ ] UI smoke covers the new panel at both viewport sizes and the
