...
- **Story:** WLA-22-01 - Map the solution and contract the briefing
- **Status:** done
- **Date:** 2026-07-15

## Proof

The repository now has two durable front-door documents:
`docs/solution-overview.md` maps the complete product, state ownership,
operating loops, trust boundaries, distribution, dated proof snapshot,
strengths, and seven observed gaps; `docs/status-briefing.md` binds the
Phase 22 response to those gaps with an exact v1 model, project-selection
rule, fourteen-step action precedence, purity boundary, and CLI/MCP/HTTP
parity contract. The root README links both, and the roadmap README no
longer reports Phase 18/v1.8.0 as current after Phase 21/v1.14.0.

The captured run below proves 208 core tests, all 360 Markdown files,
canon parity, roadmap self-validation, and whitespace checks. It was
captured before the mechanical done flip, so its index tree is the
pre-finalization tree as designed; the evidence file itself ships with
the story flip.

### Captured run — 2026-07-16T00:52:18Z

- **Command:** `bash -c python3 pmo-roadmap/tests/dw-core-tests.py && pmo-roadmap/tests/docs-lint.sh && pmo-roadmap/tests/canon-lint.sh && .githooks/dw check work-log-automation && git diff --check`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 388af61171414e5d1d08feb5aa238c46f304b3db

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest.test_codex_flag_opt_out_respected) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.a2xcae32/config.toml; respecting the opt-out
ok
...
test_pi_prompt_drift_is_a_check_error (__main__.RiderD
