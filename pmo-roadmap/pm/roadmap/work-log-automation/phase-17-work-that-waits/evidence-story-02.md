...
- **Story:** WLA-17-02 - Pause and resume a phase
- **Status:** done
- **Date:** 2026-07-11

## Proof

A phase can pause and come back. `plan_phase_pause` writes
`paused (<why> — since <date>)` into the phase file's `**Status:**`
line (an existing line keeps its bullet/bare shape; a phase that
never declared one gets the flagship's bare shape under the H1) and
into the project README's phase index row; `plan_phase_resume`
restores `in-progress` in both places and refuses a phase that is
not actually paused. Both refuse a closed phase (final-summary.md).
The read layer answers through `phase_header_status` /
`phase_is_paused`; project context carries `paused` + `pause_note`
per phase, the workbench project summary counts `paused_phase_count`,
and the preview→apply editor gained `pause_phase` / `resume_phase`
mutation kinds on the same fingerprint discipline. The rail event
taxonomy grew `phase_paused` / `phase_resumed` (docs/mission-control
§3 updated). A paused phase stays *open* — never closed, no new
`dw check` issues.

Two captured runs below, both authoritative:

1. **16:11:14Z** — the full core suite: 192 tests (was 188; four new
   WLA-17-02 cases: pause→resume round trip through header, README
   row, context, and check; bare-status insertion under the H1;
   the four refusals by name; workbench preview→apply for both new
   kinds), exit 0.
2. **16:11:25Z** — the live CLI walk on a scratch roadmap: bare
   pause refused; paused header + README row shown verbatim;
   `dw check` clean and context reading `paused: True` with the
   note while `active` stays true; resume restoring both places;
   resume-unpaused and pause-closed refused by name.

Also green this session (not captured): gate-parity.sh, mcp-server.sh,
roadmap-cli.sh, workbench-explorer.sh, package-smoke.sh, docs-lint.sh.

### Captured run — 2026-07-11T16:11:14Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** f2ead9255ccb8303ec461d213b73a261ff6d11c1

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.9ei58rnf/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 192 tests in 11.264s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.5lq91a9l/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.5lq91a9l/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.4epnly5u/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.p468odey/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.p468odey/settings.json
```

### Captured run — 2026-07-11T16:11:25Z

- **Command:** `/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/475abfb9-3a84-4a46-9535-cd63c3b7d3fd/scratchpad/wla-17-02-demo.sh`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** f2ead9255ccb8303ec461d213b73a261ff6d11c1

```text
== 1. pause without a reason is refused ==
dw: a pause needs its why; pass --reason
exit=1 (expected 1)

== 2. pause with a reason lands in the header AND the README row ==
phase-1	paused	phase 2 takes priority
exit=0
# Phase 1 - Alpha

**Status:** paused (phase 2 takes priority — since 2026-07-11)

| 1 | Alpha | paused (phase 2 takes priority — since 2026-07-11) | [phase-1-alpha](./phase-1-alpha/) |

== 3. dw check stays clean; context reads paused + note ==
dw check: ok
check exit=0
paused: True | note: phase 2 takes priority — since 2026-07-11 | active: True

== 4. resume restores in-progress in both places ==
phase-1	in-progress	(was: paused (phase 2 takes priority — since 2026-07-11))
exit=0
# Phase 1 - Alpha
...
== 5. resuming an unpaused phase is refused by name ==
dw: phase 1 is not paused (status: in-progress)
exit=1 (expected 1)

== 6. a closed phase cannot pause ==
dw: phase 1 is closed (final-summary.md exists); a closed phase cannot pause
exit=1 (expected 1)
```
