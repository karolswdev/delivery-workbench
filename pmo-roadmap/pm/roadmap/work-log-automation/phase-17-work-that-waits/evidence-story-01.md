...
- **Story:** WLA-17-01 - on-hold enters the write vocabulary; every park carries a reason
- **Status:** done
- **Date:** 2026-07-11

## Proof

The hold vocabulary is live end to end. `HOLD_STATUSES = {on-hold,
paused}` joined `STORY_OPEN_STATUSES` (model.py); `plan_story_status`
takes `reason` and writes it as cell/header decoration
(`on-hold (<why> — since <date>)`) that `normalize_status` reads
through and the new `status_note` recovers; a bare park is refused; a
done with a reason is refused (a decorated done would evade the
gate's exact flip detection). The `--reason` flag rides the CLI, the
`reason` param rides MCP `dw_story_status` and the workbench
`update_story_status` mutation body. Docs moved with the code:
roadmap-builder §2.3 (declared | done-synonyms | hold-synonym, parity
test extended to three groups), CLAUDE-snippet + regenerated
CLAUDE.md block, MCP enum text. `story_context` now carries
`status_token` + `status_note` for every read surface.

Two captured runs below, both authoritative:

1. **16:00:19Z** — the full core suite: 188 tests (was 183; five new
   WLA-17-01 cases: bare-park refusal, reason round-trip through
   write→parse→normalize→note, open-status composition + done
   refusal, byte-identical plain writes, status_note extraction
   table; plus three new normalize pins incl. decorated
   `on-hold`/`paused` and `ON HOLD`), exit 0.
2. **16:00:38Z** — the live CLI walk on a scratch roadmap: bare park
   refused by name; parked cell reads
   `on-hold (pivot to the board work — since 2026-07-11)` in both
   the table and the story header; `dw check` clean on the decorated
   pair; `done --reason` refused; `paused` synonym gates identically.

Also green this session (not captured): gate-parity.sh (gate
untouched), mcp-server.sh, roadmap-cli.sh, canon-lint.sh,
docs-lint.sh, agent-surface.sh, package-smoke.sh.

### Captured run — 2026-07-11T16:00:19Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 4476f3651cf9dd904e97c930947d59a2cfba9075

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.7s8j09r8/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 188 tests in 10.118s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.1aotbhea/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.1aotbhea/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.hanp96hc/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.w7rdx60l/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.w7rdx60l/settings.json
```

### Captured run — 2026-07-11T16:00:38Z

- **Command:** `/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/475abfb9-3a84-4a46-9535-cd63c3b7d3fd/scratchpad/wla-17-01-demo.sh`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 4476f3651cf9dd904e97c930947d59a2cfba9075

```text
== 1. bare park is refused ==
dw: a hold needs its why; pass --reason when parking a story on-hold/paused
exit=1 (expected 1)

== 2. park with a reason lands as decoration ==
DM-1-01	on-hold	pm/roadmap/demo/phase-1-alpha/story-01-first.md
exit=0
| DM-1-01 | First thing | on-hold (pivot to the board work — since 2026-07-11) | [story-01-first](./story-01-first.md) | - |
- **Status:** on-hold (pivot to the board work — since 2026-07-11)

== 3. dw check stays clean on the decorated pair ==
dw check: ok
exit=0

== 4. done takes no reason ==
dw: done takes no --reason; the evidence file is the reason
exit=1 (expected 1)

== 5. paused gates as the on-hold synonym ==
DM-1-01	paused	pm/roadmap/demo/phase-1-alpha/story-01-first.md
exit=0
| DM-1-01 | First thing | paused (waiting on the model drop — since 2026-07-11) | [story-01-first](./story-01-first.md) | - |
```
