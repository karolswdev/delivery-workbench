...
- **Story:** WLA-17-05 - The board on the workbench
- **Status:** done
- **Date:** 2026-07-11

## Proof

The board has glass. `GET /api/projects/<slug>/board` serves the
WLA-17-04 model through the standard envelope (route covered by a
handle_api test, unknown project → 400); `#/board[/slug]` renders it:
one swimlane per open phase (pointer lane leading), six status
columns with counts, cards linking to the story view (id, title,
✓ evidence receipt, and the hold reason on the card for
blocked/on-hold); paused lanes dimmed + dashed with a ⏸ reason
banner; closed lanes folded into `<details>` one-line receipts under
a "closed phases (N)" section — click to expand, so a 90-phase tree
stays one light page. A "board" link rides the topbar and the
project view's Phases heading. The viewport smoke harness grew the
board view (now 7 views × desktop + mobile) and passes with real
headless-Firefox renders.

Screenshots (captured from live servers, in [assets/](./assets/)):

- [board-demo-paused-holds.png](./assets/board-demo-paused-holds.png)
  — a demo tree: blocked card carrying "waiting on upstream API
  keys — since 2026-07-11" in amber, done card green with ✓, and
  phase 2 dimmed/dashed reading "⏸ paused — phase 1 must land
  first — since 2026-07-11".
- [board-this-repo.png](./assets/board-this-repo.png) — this repo's
  real board mid-phase: WLA-17-05 in-progress on its own lane,
  four ✓ done cards, 17 closed phases folded.

Two captured runs below, both authoritative:

1. **16:32:22Z** — the full core suite: 199 tests (was 198; new
   workbench board-route case pinning envelope, columns, done
   receipts, and the 400), exit 0.
2. **16:32:34Z** — the live server: curl of
   /api/projects/work-log-automation/board printing ok:true, the
   six columns, and lane 0 (phase 17, unpaused, four done ids).

Also green this session (not captured): workbench-explorer.sh,
workbench-ui-smoke.sh (14 viewport renders incl. the board).

### Captured run — 2026-07-11T16:32:22Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 3337ab5b5915ed62990c1e3d2102b3b42b1d3863

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.2r75q_ah/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 199 tests in 11.295s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.00d1bv_w/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.00d1bv_w/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.w96j57nn/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.yjxdu9e3/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.yjxdu9e3/settings.json
```

### Captured run — 2026-07-11T16:32:34Z

- **Command:** `bash -c pmo-roadmap/bin/dw-workbench --root . --port 23179 --quiet & SRV=$!; sleep 1; curl -s http://127.0.0.1:23179/api/projects/work-log-automation/board | /usr/bin/python3 -c "import json,sys; d=json.load(sys.stdin); m=d['data']; print('ok:', d['ok']); print('columns:', m['columns']); lane=m['phases'][0]; print('lane 0: phase', lane['number'], lane['slug'], '| paused:', lane['paused'], '| done:', [c['story_id'] for c in lane['columns']['done']])"; kill $SRV`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 3337ab5b5915ed62990c1e3d2102b3b42b1d3863
...
lane 0: phase 17 work-that-waits | paused: False | done: ['WLA-17-01', 'WLA-17-02', 'WLA-17-03', 'WLA-17-04']
dw-workbench: shutting down
dw-workbench: serving /Users/karol/dev/code/delivery-workbench
dw-workbench: http://127.0.0.1:23179/ (localhost or your own .ts.net tailnet; Ctrl-C to stop)
