...
- **Story:** WLA-17-03 - next tells the truth about parked work; dw holds is the ledger
- **Status:** done
- **Date:** 2026-07-11

## Proof

Parked work now has one review surface and `next` stopped being
silent about it. `next_story` skips paused phases entirely (the
pause is a recorded decision, honored); `parked_summary` /
`parked_headline` derive the ledger (paused phases + blocked/on-hold
stories in open phases, each with note and location); `dw next`
exit-2 and MCP `dw_next` name the counts ("parked: 1 blocked, 1
on-hold, 1 phase paused — see dw holds"); the new `dw holds
[project] [--json]` prints one greppable line per hold (exit 0 with
holds, 2 empty); project context carries `parked`; and a park with
no visible reason raises a `dw check` warning — never an error, so
legacy trees keep passing.

Two captured runs below, both authoritative:

1. **16:17:47Z** — the full core suite: 194 tests (was 192; two new
   WLA-17-03 cases: next-skips-parked at both story and phase level
   with headline counts, and bare-park warns→reason clears), exit 0.
2. **16:17:58Z** — the live walk: a scratch tree with one of
   everything parked (bare legacy block, reasoned on-hold, paused
   phase hiding an in-progress story) — `dw next` exits 2 naming
   all three; the ledger lists them greppably and as JSON; check
   stays ok with the bare block warned; releasing the hold makes
   `next` propose again. Then the flagship tree read-only:
   `dw holds holdspeak` surfaced a REAL forgotten hold — HS-25-07,
   blocked, no reason recorded, sitting in phase 25 of a project
   now working phase 93. The failure mode this story exists to end,
   demonstrated on market data.

Also green this session (not captured): gate-parity.sh,
mcp-server.sh, roadmap-cli.sh, package-smoke.sh.

### Captured run — 2026-07-11T16:17:47Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** c28bd609acf464300a99cd4e67019f2b4ca21625

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test._c6om2tw/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 194 tests in 10.892s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.hawn8mq4/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.hawn8mq4/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.ki1l5su4/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.gm0xmoda/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.gm0xmoda/settings.json
```

### Captured run — 2026-07-11T16:17:58Z

- **Command:** `/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/475abfb9-3a84-4a46-9535-cd63c3b7d3fd/scratchpad/wla-17-03-demo.sh`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** c28bd609acf464300a99cd4e67019f2b4ca21625

```text
== 1. park DM-1-02 with a reason, pause phase 2 ==
DM-1-02	on-hold	pm/roadmap/demo/phase-1-alpha/story-02-second.md
phase-2	paused	phase 1 must land first

== 2. dw next skips ALL of it and names the counts (exit 2) ==
dw next: nothing actionable (no in-progress, ready, or backlog stories); parked: 1 blocked, 1 on-hold, 1 phase paused — see dw holds
exit=2 (expected 2)

== 3. dw holds — the ledger, one greppable line per hold ==
PAUSED	phase-2-beta	phase 1 must land first — since 2026-07-11
BLOCKED	DM-1-01	(no reason recorded)	phase-1-alpha
ON-HOLD	DM-1-02	pivot to the launch — since 2026-07-11	phase-1-alpha
exit=0 (expected 0)
...
== 4. dw holds --json ==
{
    ...
    "parked_stories": [
        ...
        {
            "note": "pivot to the launch \u2014 since 2026-07-11",
            "phase": 1,
            "phase_path": "phase-1-alpha",
            "phase_paused": false,
            "status": "on-hold",
            "story_id": "DM-1-02",
            "title": "Deliberate park"
        }
    ],
    "paused_phases": [
        {
            "note": "phase 1 must land first \u2014 since 2026-07-11",
            "phase": 2,
            "phase_path": "phase-2-beta"
        }
    ]
}

== 5. the bare legacy block warns (never errors) ==
dw check: ok
check exit=0
warnings: ['multiple open phases detected: phase-1-alpha, phase-2-beta', 'parked without a recorded reason (dw story status --reason): DM-1-01']

== 6. release the hold: next proposes again ==
DM-1-02	ready	pm/roadmap/demo/phase-1-alpha/story-02-second.md
DM-1-02	ready	phase-1-alpha	Deliberate park
exit=0 (expected 0)

== 7. the flagship tree, read-only: dw holds on HoldSpeak ==
BLOCKED	HS-25-07	(no reason recorded)	phase-25-trust-and-hardening
exit=0
```
