...
- **Story:** WLA-18-02 - dw story show — one story, whole
- **Status:** done
- **Date:** 2026-07-11

## Proof

One story, whole, from one core. `api.story_detail(project, phase,
selector, root)` assembles everything `story_context` carries plus
the story and evidence markdown bodies, the parsed captured runs
(timestamp / command / exit per run), and the WLA-18-01
`paths`/`links` self-description; absences render as honest empties.
The workbench story route (`/api/projects/<slug>/stories/<id>`)
refactored onto it — additive-only shape change, existing route
consumers untouched — and the new `dw story show <project> <phase>
<story> [--json]` renders it for humans (greppable fact-per-line
header, captured-runs list, bodies in trace order) or machines. All
`find_story` selector forms work (id, number, zero-padded,
filename, stem).

Two captured runs below, both authoritative:

1. **18:04:47Z** — the full core suite: 204 tests (was 201; three
   new WLA-18-02 cases: whole-detail shape + selector forms +
   honest absences, captured-runs round trip through a real
   `run_capture`, and route-serves-the-core with the 404), exit 0.
2. **18:05:01Z** — the live CLI browse of a real story
   (WLA-17-03): status/phase/receipts/api lines, both phase-17
   captured runs listed with exit 0, then the `--json` key
   inventory showing the one object (bodies, captured_runs, paths,
   links, status_token/note, trace).

Also green this session (not captured): workbench-explorer.sh,
roadmap-cli.sh.

### Captured run — 2026-07-11T18:04:47Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 866042bac4d481342bf7794e74d5ab797d4b2af5

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.xtysdlum/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 204 tests in 13.755s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.llxdn11n/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.llxdn11n/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.8oouldoh/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.s44o43bn/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.s44o43bn/settings.json
```

### Captured run — 2026-07-11T18:05:01Z

- **Command:** `bash -c pmo-roadmap/bin/dw story show work-log-automation 17 3 | head -11; echo; echo '== json keys =='; pmo-roadmap/bin/dw story show work-log-automation 17 3 --json | /usr/bin/python3 -c "import json,sys; print(sorted(json.load(sys.stdin).keys()))"`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 866042bac4d481342bf7794e74d5ab797d4b2af5
...
status	done
phase	17 (phase-17-work-that-waits)
story	pmo-roadmap/pm/roadmap/work-log-automation/phase-17-work-that-waits/story-03-holds-ledger.md
evidence	pmo-roadmap/pm/roadmap/work-log-automation/phase-17-work-that-waits/evidence-story-03.md
api	/api/projects/work-log-automation/stories/WLA-17-03	/api/projects/work-log-automation/trace/WLA-17-03
captured runs:
  - 2026-07-11T16:17:47Z `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py` exit 0
  - 2026-07-11T16:17:58Z `/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/475abfb9-3a84-4a46-9535-cd63c3b7d3fd/scratchpad/wla-17-03-demo.sh` exit 0
--- story ---
# WLA-17-03 - next tells the truth about parked work; dw holds is the ledger

== json keys ==
['captured_runs', 'evidence', 'evidence_exists', 'evidence_file', 'evidence_markdown', 'evidence_path', 'header_status', 'links', 'paths', 'phase_number', 'status', 'status_note', 'status_token', 'story_exists', 'story_file', 'story_id', 'story_markdown', 'story_path', 'title', 'trace']
```
