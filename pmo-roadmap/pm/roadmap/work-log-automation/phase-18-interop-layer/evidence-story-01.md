...
- **Story:** WLA-18-01 - Self-describing cards: the board and the ledger carry their receipts
- **Status:** done
- **Date:** 2026-07-11

## Proof

Every card and every hold is a door now. Board cards carry `paths`
(repo-relative story / evidence / phase-status receipts — the
evidence *address* emitted even before the file exists, paired with
`evidence_exists` for the truth about occupancy) and `links` (the
workbench routes for story and trace); lanes carry
`links.phase` + `paths.phase_status`; `parked_summary` (the ledger
behind `dw holds`) carries the same for parked stories and paused
phases. All link shapes are minted in ONE place (`board.story_links`
/ `phase_links` / `story_paths`) so a route rename cannot strand a
stale shape in one surface, and the board model is stamped
`kind: delivery-workbench-board`, `schema_version: 1`.

Two captured runs below, both authoritative:

1. **18:00:30Z** — the full core suite: 201 tests (was 199; two new
   WLA-18-01 cases: the receipts/links shape pinned on board +
   ledger including the stable-evidence-address rule, and the
   no-rot walk — every emitted link resolved through
   `workbench.handle_api` and asserted 200), exit 0.
2. **18:00:44Z** — the live walk over real HTTP: a consumer holding
   only the board JSON reads the stamp, takes card WLA-17-01,
   follows `links.story` (story + evidence markdown in one hop) and
   `links.trace` (the intent-to-proof chain, all five hops
   present, shipped: True) — zero tree knowledge required.

### Captured run — 2026-07-11T18:00:30Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 5e40e9d2ed96821feecd146ca0d63fa3c7413bc6

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.xgi5uf2y/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 201 tests in 14.062s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.hllc8bmz/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.hllc8bmz/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.k8y5rlow/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.0ub3dywg/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.0ub3dywg/settings.json
```

### Captured run — 2026-07-11T18:00:44Z

- **Command:** `/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/475abfb9-3a84-4a46-9535-cd63c3b7d3fd/scratchpad/wla-18-01-demo.sh`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 5e40e9d2ed96821feecd146ca0d63fa3c7413bc6

```text
== 1. fetch the board; it names itself ==
kind: delivery-workbench-board | schema_version: 1
card: WLA-17-01 | evidence_exists: True
paths: {
  "evidence": "pmo-roadmap/pm/roadmap/work-log-automation/phase-17-work-that-waits/evidence-story-01.md",
  "phase_status": "pmo-roadmap/pm/roadmap/work-log-automation/phase-17-work-that-waits/current-phase-status.md",
  "story": "pmo-roadmap/pm/roadmap/work-log-automation/phase-17-work-that-waits/story-01-hold-vocabulary.md"
}
links: {
  "story": "/api/projects/work-log-automation/stories/WLA-17-01",
  "trace": "/api/projects/work-log-automation/trace/WLA-17-01"
}

== 2. follow links.story: the story AND its evidence, one hop ==
story_id: WLA-17-01 | status: done
story_markdown head: # WLA-17-01 - on-hold enters the write vocabulary; every park carries a reason
evidence_markdown head: # Evidence - WLA-17-01

== 3. follow links.trace: the intent-to-proof chain ==
shipped: True
  readme: exists=True
  phase_status: exists=True
  story: exists=True
  evidence: exists=True
  final_summary: exists=True

== 4. dw holds --json entries carry the same doors ==
parked entries: 0 | paused phases: 0
dw-workbench: shutting down
dw-workbench: serving /Users/karol/dev/code/delivery-workbench
dw-workbench: http://127.0.0.1:23301/ (localhost or your own .ts.net tailnet; Ctrl-C to stop)
