...
- **Story:** WLA-18-04 - The interop contract, versioned and pinned
- **Status:** done
- **Date:** 2026-07-11

## Proof

The read surface has one name: `docs/interop.md` — the versioning
stance (stamps bump deliberately, additive changes don't), the five
stamped models in one table, and one table per transport: eleven
CLI machine verbs, thirteen workbench GET routes, twelve MCP tools
(browse inventory here, full schemas by reference in mcp.md). Write
paths are referenced, never restated — one canon per concern.
Cross-links landed from mcp.md and mission-control.md §1. The pin:
`test_interop_doc_names_every_surface` derives the route inventory
from `handle_api`'s own source, the tool inventory from the MCP
registry, checks the CLI verb list and the stamped-model names, and
proves it bites with a planted surface that must read as missing.

Two captured runs below, both authoritative:

1. **18:16:10Z** — the full core suite: 208 tests (was 207; the
   parity test with its planted-omission self-check), exit 0.
2. **18:16:24Z** — the consumer's walk, using only what the doc
   names: `dw board --json` → the declared stamp verified → a
   card's `links.story` followed over HTTP → the envelope stamp
   verified → the evidence read (fittingly, WLA-18-01's own) →
   the same story through `dw story show --json` → the parity test
   run standalone, OK.

Also green this session (not captured): canon-lint.sh,
docs-lint.sh, docs-snippet-smoke.sh, mcp-server.sh, gate-parity.sh,
package-smoke.sh.

### Captured run — 2026-07-11T18:16:10Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** c8c5c07f810b4a9bac1f9b6ea2cd1a56f814c296

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.2fcofqfy/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 208 tests in 13.874s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.2n4pbkch/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.2n4pbkch/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.ldgzfynu/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.bmp6wf25/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.bmp6wf25/settings.json
```

### Captured run — 2026-07-11T18:16:24Z

- **Command:** `/private/tmp/claude-501/-Users-karol-dev-code-delivery-workbench/475abfb9-3a84-4a46-9535-cd63c3b7d3fd/scratchpad/wla-18-04-demo.sh`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** c8c5c07f810b4a9bac1f9b6ea2cd1a56f814c296

```text
== the doc names 'dw board --json' -> fetch, verify the stamp the doc declares ==
stamp ok: delivery-workbench-board v 1
card: WLA-18-01 | links: {'story': '/api/projects/work-log-automation/stories/WLA-18-01', 'trace': '/api/projects/work-log-automation/trace/WLA-18-01'}

== the doc names the story route -> follow links.story, read the evidence ==
envelope stamp ok | WLA-18-01 | captured runs: 2
evidence head: # Evidence - WLA-18-01

== the doc names 'dw story show --json' -> same story, same core, from the CLI ==
WLA-18-01 | status: done | evidence_exists: True

== the pin bites: the parity test derives inventories from code ==
Ran 1 test in 0.024s

OK
dw-workbench: shutting down
dw-workbench: serving /Users/karol/dev/code/delivery-workbench
dw-workbench: http://127.0.0.1:23411/ (localhost or your own .ts.net tailnet; Ctrl-C to stop)
