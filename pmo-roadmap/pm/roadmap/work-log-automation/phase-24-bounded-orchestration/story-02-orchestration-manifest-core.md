...
- [x] Valid representative and minimal scores compile to exact-key, stamped,
  canonical documents and stable hashes on Python 3.9; key/order-only JSON
  changes do not change semantics or hash.
- [x] Red fixtures refuse duplicate/missing ids, dangling refs, success cycles,
...
- [x] Built-in agent role presets include research/synthesis/implementation/
  review/verification/documentation/repair without hard-coding capability;
  custom roles round-trip.
- [x] Simulation deterministically reports eligibility waves, fan-out/fan-in,
...
- [x] CLI JSON/human output, installed package, docs/interop inventory, full
...
Implemented in `dw_pmo.orchestration` with a closed schema at every object
level, deterministic normalization and simulation, contained discovery,
stable semantic/document hashes, structured diagnostics, four CLI reads, and
the ordinary `research-build-review.json` preset. The source and vendored
cores are byte-aligned; the installed-wheel smoke validates and simulates the
preset. No grant, run directory, event, subprocess, or agent path is present
in this story's compiler.
