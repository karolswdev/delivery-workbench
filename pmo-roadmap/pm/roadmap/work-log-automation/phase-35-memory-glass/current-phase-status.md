...
WLA-35-02 done (2026-08-01): `memory_recall.py` exists — pure deterministic recall with seven source kinds, additive integer ranking, 32 KiB budget with whole-item drops, five typed exclusion reasons, audience slices, honest empty recall; packet suite 8 → 14 tests. WLA-35-01 landed the memory contract earlier the same day (knowledge suite 22 → 28). Next: WLA-35-03 (recall before dispatch in both conductors).
