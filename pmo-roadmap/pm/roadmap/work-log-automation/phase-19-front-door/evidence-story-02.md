...
- **Story:** WLA-19-02 - The README catches up with the shipped surface
- **Status:** done
- **Date:** 2026-07-11
...
1. **19:34:50Z — the core suite** (208 tests OK, including
   DocsLintTest over the edited README): the contract's
   `--tests-capture` run.
2. **19:35:16Z — the census walk**: the MCP tool list is DERIVED
   from `mcpserver.py`'s registrations (12 tools), and each one is
   found in the README — the stated count ("twelve tools") matches;
   the three new CLI table rows (`dw board`, `dw holds`,
   `dw story show`) are shown at their line numbers;
   `docs/interop.md` is linked; and the two rot-prone literals are
   proven gone (`Current version:` and "sixteen phases" both grep
   empty — the PyPI badge now states the version, the phase count
   is no longer written down).

Also in this change, per the story scope: the web-view section
names the `#/board` kanban and its guarded drag moves, and a short
paragraph after the CLI table introduces parked work
(on-hold-with-reason, phase pause/resume, an honest `dw next`) and
the receipts-and-links walk with the interop contract as its
reference. The version-line decision went the phase default's way:
removed, not automated.

### Captured run — 2026-07-11T19:34:50Z

- **Command:** `/usr/bin/python3 pmo-roadmap/tests/dw-core-tests.py`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 773399bae5906da4b2184bb34208e98a27b6b9b2

```text
test_codex_flag_opt_out_respected (__main__.AgentHooksTest) ... dw hook: codex_hooks is explicitly false in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.mdstlqyd/config.toml; respecting the opt-out
ok
...
----------------------------------------------------------------------
Ran 208 tests in 13.663s

OK
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.a5psuic1/settings.json
dw hook: claude — 0 installed, 4 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.a5psuic1/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.dqfm4a0j/settings.json
dw hook: claude — 4 installed, 0 already present in /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.9oc0riov/settings.json
dw hook: claude — 4 removed from /var/folders/q7/5dzz5g2116b3lq8rhg7hwjrr0000gn/T/dw-hooks-test.9oc0riov/settings.json
```

### Captured run — 2026-07-11T19:35:16Z

- **Command:** `bash -c set -e; TOOLS=$(grep -oE "^    \"dw_[a-z_]+\": \{" pmo-roadmap/lib/dw_pmo/mcpserver.py | grep -oE "dw_[a-z_]+"); COUNT=$(echo "$TOOLS" | wc -l | tr -d " "); echo "mcpserver.py registers $COUNT tools:"; echo "$TOOLS"; for t in $TOOLS; do if grep -q "$t" README.md; then echo "README names $t"; else echo "MISSING from README: $t"; exit 1; fi; done; echo "--- stated count ---"; grep -n "twelve tools" README.md; echo "--- CLI table rows ---"; grep -nE "dw (board|holds|story show)" README.md; echo "--- interop doc linked ---"; grep -n "docs/interop.md" README.md; echo "--- no hand-maintained version line ---"; if grep -n "Current version:" README.md; then echo "FAIL: version literal survives"; exit 1; else echo "none"; fi; echo "--- no stale phase count ---"; if grep -n "sixteen phases" README.md; then echo "FAIL: stale count survives"; exit 1; else echo "none"; fi`
- **Cwd:** .
- **Exit code:** 0
- **Index-tree:** 773399bae5906da4b2184bb34208e98a27b6b9b2

```text
mcpserver.py registers 12 tools:
dw_context
dw_next
dw_check
dw_doctor
...
dw_evidence_capture
dw_contract_new
README names dw_context
README names dw_next
README names dw_check
README names dw_doctor
README names dw_verify
README names dw_gate
README names dw_board
README names dw_holds
README names dw_story_show
README names dw_story_status
README names dw_evidence_capture
README names dw_contract_new
--- stated count ---
166:up automatically. The server exposes twelve tools backed by the same
--- CLI table rows ---
144:| `dw board [--json]` | The kanban in the terminal: a swimlane per phase, six status columns, evidence ticks. |
145:| `dw holds [--json]` | The ledger of parked work: every on-hold story and paused phase, each with its recorded reason. |
146:| `dw story show <p> <ph> <st> [--json]` | One story whole: header, status and why, story and evidence bodies, captured runs, receipts. |
--- interop doc linked ---
160:[docs/interop.md](./docs/interop.md).
255:- [The interop contract](./docs/interop.md): every read surface over CLI, HTTP, and MCP, with its schema version
--- no hand-maintained version line ---
none
--- no stale phase count ---
none
```
