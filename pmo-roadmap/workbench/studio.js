...
function deliveryChoiceCard(choice) {
  ...
  return `<article class="delivery-choice${selected ? " selected" : ""}" data-choice="${esc(choice.id)}">
    <div class="delivery-choice-head"><span class="ops-label">${esc(choice.tier === "vanilla" ? "ordinary roadmap" : choice.tier === "bounded-run" ? "one bounded delivery" : "optional delivery program")}</span>${badge(choice.readiness, choice.available ? "ok" : "warn")}</div>
...
function renderDeliverySetup() {
  ...
  app.innerHTML = `${destinationNav("plan", "#/program-studio")}<div class="delivery-setup" data-readiness="${esc(model.readiness)}">
    <header class="delivery-setup-hero"><div><span class="ops-label">Plan your delivery</span><h1>What are you delivering?</h1><p>${esc(model.summary)} Nothing starts until you review and confirm it.</p></div></header>
    <section class="delivery-scope" aria-labelledby="delivery-scope-title"><div><span class="ops-label">Step 1</span><h2 id="delivery-scope-title">Choose the delivery scope</h2></div>
      ...
      <div class="delivery-current-work"><span class="ops-label">Current work</span><strong>${esc(work?.title || "Choose a project to see current work")}</strong><small>${work ? `${esc(work.story_id)} · ${esc(work.status)}` : "No work is inferred."}</small></div>
    </section>
    ...
    <section class="delivery-choice-step" aria-labelledby="delivery-choice-title"><div><span class="ops-label">Step 2</span><h2 id="delivery-choice-title">Choose the operating mode</h2><p>Compare all three. A higher mode is never selected or started for you.</p></div>
...
function studioTechnicalView() {
  ...
  return `<div class="studio-technical-view"><header><div><span class="orch-eyebrow ops-label">Technical details</span><h2>${team ? "Exact responsibilities, provenance, and configuration" : "Exact graph, fields, and configuration"}</h2><p>${team ? "Inspect stable role IDs, candidate profiles, provider and model resolution, auth and principal fingerprints, work areas, sessions, packet bounds, decision rules, and the lossless source." : "Use this view for hierarchical flows, bounded loops, discussion cells, exact conditions, raw import/export, and source-level diagnostics."}</p></div>${badge("same source document", "ok")}</header>
...
function studioSimulateView() {
  ...
  return `<div class="studio-simulation" data-scenario="${esc(studioState.scenario)}"><header><div><span class="orch-eyebrow ops-label">Try the delivery flow</span><h2>See how work could move before saving or requesting permission</h2><p>This is an inspection-only example. It starts no work and changes no plan.</p></div>${badge("starts nothing", "ok")}</header><div class="studio-scenario-tabs" role="tablist" aria-label="Delivery examples">${scenarios.map((scenario) => `<button type="button" id="studio-scenario-tab-${esc(scenario.id)}" role="tab" aria-controls="studio-scenario-panel" data-studio-scenario="${esc(scenario.id)}" aria-selected="${scenario.id === studioState.scenario}" tabindex="${scenario.id === studioState.scenario ? "0" : "-1"}"${scenario.available ? "" : " disabled"}>${esc(scenario.label)}</button>`).join("")}</div><div id="studio-scenario-panel" role="tabpanel" aria-labelledby="studio-scenario-tab-${esc(studioState.scenario)}">${studioScenarioSummary(studioState.scenario)}${studioSimulationDetails()}</div></div>`;
}
