...
def redeem(
    ...
    ``user_id`` is the redeemer's from.id — recorded as the
    owner-of-record, the one identity group-context consent checks
    against (phase 20: no allowlist; the pairing act names the
    owner)."""
    ...
    state.owner_user_id = user_id if isinstance(user_id, int) else None
    state.save()
